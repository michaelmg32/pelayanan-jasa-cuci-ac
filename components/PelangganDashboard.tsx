  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50 overflow-hidden relative min-h-0 h-full">
      <div className="w-full h-full flex flex-col relative">
                            ...
                            <div className="border border-indigo-200 bg-indigo-50/70 rounded-xl p-3 space-y-3 text-left">
                              ...
                              {selectedPayMethod === 'TRANSFER' && (
                                <div className="bg-white border border-indigo-100 p-3.5 rounded-lg text-[11px] text-slate-700 space-y-3 text-left">
                                  ...
                                  {((appSettings?.[selectedRegionId || 'GLOBAL']?.bank_name || appSettings?.['GLOBAL']?.bank_name) || (appSettings?.[selectedRegionId || 'GLOBAL']?.qris_image || appSettings?.['GLOBAL']?.qris_image)) && (
                                    <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 space-y-3">
                                      <div className="text-[8px] font-black uppercase text-indigo-650 tracking-wider">
                                        PILIHAN TRANSFER LANGSUNG (MANUAL)
                                      </div>

                                      {(appSettings?.[selectedRegionId || 'GLOBAL']?.bank_name || appSettings?.['GLOBAL']?.bank_name) && (
                                        <div className="space-y-1 bg-white border border-slate-150 rounded-xl p-3 shadow-xs text-left relative group">
                                          ...
                                          <div className="text-xs font-black text-slate-800 flex justify-between items-center mt-1">
                                            <span>{(appSettings?.[selectedRegionId || 'GLOBAL']?.bank_name || appSettings?.['GLOBAL']?.bank_name)}</span>
                                            <button
                                              onClick={() => {
                                                if ((appSettings?.[selectedRegionId || 'GLOBAL']?.bank_account_number || appSettings?.['GLOBAL']?.bank_account_number)) {
                                                  navigator.clipboard.writeText((appSettings?.[selectedRegionId || 'GLOBAL']?.bank_account_number || appSettings?.['GLOBAL']?.bank_account_number));
                                                  alert(`✓ Nomor rekening ${(appSettings?.[selectedRegionId || 'GLOBAL']?.bank_account_number || appSettings?.['GLOBAL']?.bank_account_number)} berhasil disalin!`);
                                                }
                                              }}
                                          ...
                                          <div className="font-mono text-sm font-black text-indigo-700 mt-1 select-all">
                                            {(appSettings?.[selectedRegionId || 'GLOBAL']?.bank_account_number || appSettings?.['GLOBAL']?.bank_account_number)}
                                          </div>
                                          <div className="text-[9.5px] font-semibold text-slate-500 mt-0.5">
                                            a.n. {(appSettings?.[selectedRegionId || 'GLOBAL']?.bank_account_holder || appSettings?.['GLOBAL']?.bank_account_holder)}
                                          </div>
                                        </div>
                                      )}

                                      {(appSettings?.[selectedRegionId || 'GLOBAL']?.qris_image || appSettings?.['GLOBAL']?.qris_image) && (
                                        <div className="bg-white border border-slate-150 rounded-xl p-3 shadow-xs text-center space-y-2">
                                          ...
                                          <div className="flex justify-center py-1">
                                            <img
                                              src={(appSettings?.[selectedRegionId || 'GLOBAL']?.qris_image || appSettings?.['GLOBAL']?.qris_image)}
                                              alt="QRIS Code"
        ...
        {/* NEW ORDER MODAL */}
        {showNewOrderModal && (
          <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-xs flex flex-col justify-end z-50 animate-fade-in pt-10">
            ...
            <div className="bg-white rounded-2xl w-full max-w-md max-h-[90vh] overflow-hidden shadow-2xl flex flex-col animate-scale-in" onClick={e => e.stopPropagation()}>
              <div className="p-4 bg-indigo-600 text-white flex justify-between items-center shrink-0">
                <div>
                  ...
                  <p className="text-[10px] text-indigo-200 mt-0.5 font-mono">{selectedHistoryOrder.id}</p>
                </div>
                <button onClick={() => setSelectedHistoryOrder(null)} className="p-1 hover:bg-indigo-500 rounded-full transition cursor-pointer">
                  <X size={18} />
                </button>
              </div>
              <div className="p-5 overflow-y-auto space-y-4 text-slate-700 text-xs">
                ...
                {selectedHistoryOrder.completionNotes && (
                  <div className="space-y-1 pt-2">
                    <span className="text-[9px] font-black uppercase text-slate-400 tracking-wider">Catatan Teknisi</span>
