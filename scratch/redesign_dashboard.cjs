...
// 2. Remove the three-dots menu from the top bar
const topBarStart = text.indexOf('<div className="relative">');
const topBarEnd = text.indexOf('</div>\n        </div>\n\n        {/* MAIN CONTENT AREA */}');
if (topBarStart !== -1 && topBarEnd !== -1) {
    text = text.substring(0, topBarStart) + text.substring(topBarEnd);
}

// 3. Replace the MAIN CONTENT AREA header and the old activeTab === 'dashboard' header
...
if (text.includes(oldMainContent)) {
    text = text.replace(oldMainContent, newMainContent);
}

// 4. Remove History black headers
const historyHeader = `              <div className="bg-slate-900 px-5 pt-5 pb-5 text-white text-left rounded-b-[24px]">
              ...
              <div className="px-4 py-4 space-y-4">`;
const newHistoryHeader = `              <div className="px-5 py-2 space-y-4 animate-in fade-in duration-300">`;
if (text.includes(historyHeader)) {
    text = text.replace(historyHeader, newHistoryHeader);
}

// 5. Remove Profile black headers
const profileHeader = `              <div className="bg-slate-900 px-5 pt-5 pb-5 text-white text-left rounded-b-[24px]">
...
const newProfileHeader = `              <div className="p-5 space-y-5 animate-in fade-in duration-300">
                <h3 className="font-extrabold text-slate-800 text-sm uppercase tracking-wider pl-1 mb-2">Profil Karyawan</h3>`;
if (text.includes(profileHeader)) {
    text = text.replace(profileHeader, newProfileHeader);
}

// 6. Remove Gaji black headers
const gajiHeader = `              <div className="bg-slate-900 px-5 pt-5 pb-5 text-white text-left rounded-b-[24px]">
              ...
              <div className="p-4 space-y-5">`;
const newGajiHeader = `              <div className="p-5 space-y-5 animate-in fade-in duration-300">
                <h3 className="font-extrabold text-slate-800 text-sm uppercase tracking-wider pl-1 mb-2">Gaji & Poin Member</h3>`;
if (text.includes(gajiHeader)) {
    text = text.replace(gajiHeader, newGajiHeader);
}

// 7. Add Logout Popup at the end before closing tag
const popupString = `      {showLogoutConfirm && (
...
console.log('Successfully updated dashboard with icon grid');
