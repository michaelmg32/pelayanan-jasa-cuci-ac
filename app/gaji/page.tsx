...
import type { StaffGrade, SalaryRecord, StaffWithGrade, SalarySummary } from '@/types';
...
function GradeModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (grade) {
        await updateStaffGrade(grade.id, form);
      } else {
        await createStaffGrade(form);
      }
      onSave();
      onClose();
    } catch (err: unknown) {
        ...
        <form onSubmit={handleSubmit}>
          <div className="gaji-form-group">
            ...
            <input
              ...
              placeholder="contoh: Senior, Teknisi A, Junior B..."
              required
            />
          </div>
          <div className="gaji-form-group">
            ...
            <textarea
              className="gaji-input gaji-textarea"
              value={form.description}
              onChange={e => setForm({ ...form, description: e.target.value })}
              placeholder="Keterangan singkat grade ini..."
              rows={2}
            />
          </div>
          <div className="gaji-form-row">
            ...
            <div className="gaji-form-group">
              ...
              <input
                ...
                onChange={e => setForm({ ...form, fixed_bonus: Number(e.target.value) })}
              />
            </div>
          </div>
          <div className="gaji-form-group">
            ...
            <input
              className="gaji-input"
              type="number" min="0"
              value={form.bonus_per_order}
              onChange={e => setForm({ ...form, bonus_per_order: Number(e.target.value) })}
            />
            <span className="gaji-hint">Akan dikalikan jumlah order selesai dalam sebulan</span>
          </div>
          ...
          <div className="gaji-modal-actions">
            ...
            <button type="submit" className="gaji-btn gaji-btn-primary" disabled={loading}>
              {loading ? '⏳ Menyimpan...' : '💾 Simpan Grade'}
            </button>
          </div>
        </form>
      </div>
...
function MarkPaidModal({ record, onClose, onSave }: { record: SalaryRecord; onClose: () => void; onSave: () => void }) {
  ...
  const handlePay = async () => {
    setLoading(true);
    try {
      await updateSalaryStatus(record.id, 'PAID', notes);
      onSave();
      onClose();
    ...
    <div className="gaji-modal-overlay" onClick={onClose}>
      <div className="gaji-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="gaji-modal-actions">
          ...
          <button className="gaji-btn gaji-btn-success" onClick={handlePay} disabled={loading}>
            {loading ? '⏳...' : '✅ Konfirmasi Lunas'}
          </button>
        </div>
      </div>
    </div>
...
export default function GajiPage() {
  ...
  const loadRecords = useCallback(async () => {
    ...
    try {
      setRecords(await fetchSalaryRecords(filterMonth ? { period_month: filterMonth } : undefined));
  ...
  }, [activeTab, loading, loadGrades, loadStaff, loadRecords, loadSummary]);
  ...
  const handleAssignGrade = async (staffId: string) => {
    try {
      await assignGradeToUser(staffId, assignGradeValue || null);
      setAssigningStaffId(null);
      loadStaff();
    } catch (err: unknown) {
      alert(err instanceof Error ? err.message : 'Gagal assign grade.');
    }
  };
  ...
  if (loading) return (
    <div className="gaji-loading-screen">
      <div className="gaji-spinner" />
      <p>Memuat Sistem Penggajian...</p>
    </div>
  );
    ...
    <div className="gaji-page">
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .gaji-stat-card .stat-value {
          font-size: 1.3rem;
          font-weight: 700;
          color: #f1f5f9;
        }
        .gaji-stat-card.purple { border-color: rgba(167,139,250,0.3); }
        .gaji-stat-card.purple .stat-value { color: #a78bfa; }
        .gaji-stat-card.green { border-color: rgba(52,211,153,0.3); }
        .gaji-stat-card.green .stat-value { color: #34d399; }
        .gaji-stat-card.orange { border-color: rgba(251,146,60,0.3); }
        .gaji-stat-card.orange .stat-value { color: #fb923c; }
        ...
        .gaji-loading-screen { min-height: 100vh; display: flex; flex-direction: column; align-items: center; justify-content: center; background: linear-gradient(135deg, #0f0c29, #302b63); color: white; gap: 1rem; }
        .gaji-spinner { width: 40px; height: 40px; border: 3px solid rgba(255,255,255,0.2); border-top-color: #a78bfa; border-radius: 50%; animation: spin 0.7s linear infinite; }
        ...
        {activeTab === 'grade' && (
          <>
            <div className="gaji-card">
              <div className="gaji-card-header">
                <div>
                  ...
                  <div style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.45)', marginTop: '0.25rem' }}>
                    Buat tingkatan karyawan beserta konfigurasi gaji & bonus
                  </div>
                </div>
              ...
              {gradeLoading ? (
                <div className="gaji-empty"><div className="gaji-spinner" style={{ margin: '0 auto' }} /></div>
              ) : grades.length === 0 ? (
                <div className="gaji-empty">
                  <div className="gaji-empty-icon">🏅</div>
                  <p>Belum ada grade. Buat grade pertama!</p>
                </div>
              ) : (
                <div className="gaji-grade-grid">
                  {grades.map(g => (
                    <div key={g.id} className="gaji-grade-card">
                      ...
                      {userRole === 'OWNER' && g.regionName && (
                        <div style={{ marginBottom: '0.5rem' }}>
                          <span className="gaji-region-tag">🌏 {g.regionName}</span>
                        </div>
                      )}
                      {g.description && <div className="gaji-grade-desc">{g.description}</div>}
                      <div className="gaji-salary-breakdown">
                        <div className="gaji-salary-row">
                          <span>💰 Gaji Pokok</span>
                          <span>{formatRupiah(g.base_salary || 0)}</span>
                        </div>
                        <div className="gaji-salary-row">
                          <span>🎁 Bonus Tetap</span>
                          <span>{formatRupiah(g.fixed_bonus || 0)}</span>
                        </div>
                        <div className="gaji-salary-row">
                          <span>⚡ Bonus/Order</span>
                          <span>{formatRupiah(g.bonus_per_order || 0)}</span>
                        </div>
                        <div className="gaji-salary-row total">
                          <span>Total (tanpa order bonus)</span>
                          <span>{formatRupiah((g.base_salary || 0) + (g.fixed_bonus || 0))}</span>
                        </div>
                      </div>
            ...
            <div style={{ background: 'rgba(96,165,250,0.08)', border: '1px solid rgba(96,165,250,0.2)', borderRadius: '12px', padding: '1rem 1.25rem', fontSize: '0.83rem', color: 'rgba(255,255,255,0.6)' }}>
              <strong style={{ color: '#60a5fa' }}>ℹ️ Cara Kerja Bonus:</strong>
              <br />Total Gaji = Gaji Pokok + Bonus Tetap + (Bonus per Order × Jumlah Order Selesai di Bulan Itu)
            </div>
          </>
        )}
          ...
          <div className="gaji-card">
            <div className="gaji-card-header">
              <div>
                ...
                <div style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.45)', marginTop: '0.25rem' }}>
                  Tetapkan tingkatan grade untuk setiap karyawan (teknisi)
                </div>
              </div>
            </div>
                        ...
                        {assigningStaffId === s.id ? (
                          <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'wrap' }}>
                            <select
                              ...
                              value={assignGradeValue}
                              onChange={e => setAssignGradeValue(e.target.value)}
                            >
                          ...
                          <button className="gaji-btn gaji-btn-ghost gaji-btn-sm" onClick={() => {
                            setAssigningStaffId(s.id);
                            setAssignGradeValue(s.grade_id || '');
                          }}>✏️ Atur Grade</button>
                        )}
                      </td>
                    </tr>
        ...
        {activeTab === 'proses' && (
          <>
            <div className="gaji-card">
              ...
              <div className="gaji-filter-row">
                ...
                <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'flex-end' }}>
                  <button className="gaji-btn gaji-btn-ghost" onClick={handlePreview} disabled={previewLoading}>
                    {previewLoading ? '⏳ Menghitung...' : '👁️ Preview Kalkulasi'}
                  </button>
                </div>
              </div>

              {generateMsg && (
                <div className={generateMsg.startsWith('✅') ? 'gaji-success-msg' : 'gaji-error-msg'}>
                  {generateMsg}
                </div>
              )}
                ...
                <>
                  <div style={{ marginBottom: '1rem' }}>
                    ...
                    <span style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.45)', marginLeft: '0.75rem' }}>
                      Preview kalkulasi — belum disimpan
                    </span>
                  </div>
                  <div className="gaji-table-wrap">
                    <table className="gaji-table">
                      ...
                      <tbody>
                        {preview.map((r, i) => (
                          <tr key={i}>
                            ...
                            <td>
                              <span className="gaji-grade-badge" style={{ fontSize: '0.72rem', padding: '0.15rem 0.5rem' }}>
                                {r.grade_name}
                              </span>
                            </td>
                            <td>{formatRupiah(r.base_salary)}</td>
                            <td style={{ textAlign: 'center' }}>
                              <span style={{ background: 'rgba(96,165,250,0.15)', color: '#60a5fa', padding: '0.15rem 0.5rem', borderRadius: '6px', fontSize: '0.8rem' }}>
                                {r.total_orders_completed}
                              </span>
                            </td>
                            <td>{formatRupiah(r.order_bonus)}</td>
                            <td>{formatRupiah(r.fixed_bonus)}</td>
                            <td>
                              <strong style={{ color: '#a78bfa' }}>{formatRupiah(r.total_salary)}</strong>
                            </td>
                          </tr>
                        ))}
                      </tbody>
                  ...
                  <div className="gaji-preview-total">
                    ...
                    <button
                      ...
                      disabled={generating}
                    >
                      {generating ? '⏳ Menyimpan...' : '🔒 Kunci & Simpan Gaji'}
                    </button>
                  </div>
                </>
              )}
            ...
            <div style={{ background: 'rgba(251,146,60,0.08)', border: '1px solid rgba(251,146,60,0.2)', borderRadius: '12px', padding: '1rem 1.25rem', fontSize: '0.83rem', color: 'rgba(255,255,255,0.6)' }}>
              <strong style={{ color: '#fb923c' }}>⚠️ Perhatian:</strong> Setelah gaji dikunci & disimpan, slip yang sudah ditandai <strong>LUNAS</strong> tidak akan bisa ditimpa ulang. Pastikan data sudah benar sebelum menyimpan.
            </div>
          </>
        )}
        ...
        {activeTab === 'riwayat' && (
          <>
            <div className="gaji-stats">
              <div className="gaji-stat-card">
                <div className="stat-label">Total Slip</div>
                <div className="stat-value">{records.length}</div>
              </div>
              <div className="gaji-stat-card green">
                <div className="stat-label">✅ Sudah Lunas</div>
                <div className="stat-value">{records.filter(r => r.status === 'PAID').length}</div>
              </div>
              <div className="gaji-stat-card orange">
                <div className="stat-label">⏳ Belum Lunas</div>
                <div className="stat-value">{records.filter(r => r.status === 'PENDING').length}</div>
              </div>
              <div className="gaji-stat-card purple">
                <div className="stat-label">💰 Total Lunas</div>
                <div className="stat-value">{formatRupiah(totalPaid)}</div>
              </div>
              <div className="gaji-stat-card">
                <div className="stat-label">⏳ Total Pending</div>
                <div className="stat-value">{formatRupiah(totalPending)}</div>
              </div>
            </div>
            ...
            <div className="gaji-card">
              <div className="gaji-card-header">
                ...
                <div className="gaji-filter-row" style={{ margin: 0 }}>
                  ...
                  />
                  <button className="gaji-btn gaji-btn-ghost" onClick={loadRecords} disabled={recordsLoading}>
                    {recordsLoading ? '⏳' : '🔄 Muat'}
                  </button>
                </div>
              </div>
              ...
              <div className="gaji-table-wrap">
                <table className="gaji-table">
                  ...
                  <tbody>
                    {records.length === 0 ? (
                      <tr>
                        <td colSpan={10} style={{ textAlign: 'center', padding: '2.5rem', opacity: 0.5 }}>
                          Belum ada riwayat slip gaji. Proses gaji di tab "Proses Gaji".
                        </td>
                      </tr>
                    ) : records.map(r => (
                      <tr key={r.id}>
                        ...
                        <td>
                          <span className="gaji-month-display" style={{ fontSize: '0.8rem', padding: '0.3rem 0.6rem' }}>
                            {formatMonth(r.period_month)}
                          </span>
                        </td>
                        <td>{formatRupiah(r.base_salary)}</td>
                        <td>
                          {formatRupiah(r.order_bonus)}
                          <div style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.4)' }}>
                            {r.total_orders_completed} order
                          </div>
                        </td>
                        <td>{formatRupiah(r.fixed_bonus)}</td>
                        ...
                        <td>
                          {r.status === 'PENDING' ? (
                            <button className="gaji-btn gaji-btn-success gaji-btn-sm" onClick={() => setMarkPaidRecord(r)}>
                              💳 Bayar
                            </button>
                          ) : (
          ...
          <>
            <div className="gaji-card">
              ...
              {summary.length === 0 ? (
                <div className="gaji-empty">
                  <div className="gaji-empty-icon">📊</div>
                  <p>Belum ada data gaji untuk tahun {summaryYear}.</p>
                </div>
              ) : (
                <div className="gaji-table-wrap">
                  <table className="gaji-table">
                    ...
                    <tbody>
                      {summary.map((s, i) => (
                        <tr key={i}>
                          ...
                          <td>
                            <span style={{ color: '#34d399' }}>{s.paid_count}</span>
                            <span style={{ color: 'rgba(255,255,255,0.3)' }}>/{s.total_staff}</span>
                          </td>
                          <td>
                            {Number(s.pending_count) > 0 ? (
                              <span style={{ color: '#fb923c', fontWeight: 600 }}>{s.pending_count}</span>
                            ) : (
                    ...
                    <tfoot>
                      <tr>
                        <td colSpan={5} style={{ textAlign: 'right', fontWeight: 700, color: 'rgba(255,255,255,0.7)', padding: '0.85rem 1rem' }}>
                          Grand Total Tahun {summaryYear}:
                        </td>
                        <td style={{ fontWeight: 800, color: '#a78bfa', fontSize: '1rem', padding: '0.85rem 1rem' }}>
                          {formatRupiah(summary.reduce((s, r) => s + Number(r.total_salary_cost), 0))}
                        </td>
                        <td colSpan={2} />
                      </tr>
                    </tfoot>
