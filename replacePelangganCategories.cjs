const fs = require('fs');
let content = fs.readFileSync('components/PelangganDashboard.tsx', 'utf-8');
...
// 2. Add derived variables inside the component
if (!content.includes('hasMoreCategories')) {
  content = content.replace(
    "const categories = useMemo(() =>",
    "const hasMoreCategories = (allCategories || []).filter((c: any) => c.region_id === selectedRegionId || !c.region_id).length > 8;\n  const displayCategories = hasMoreCategories && !isCategoriesExpanded ? categories.slice(0, 7) : categories;\n\n  const categories = useMemo(() =>"
  );
  // wait, categories is used in displayCategories! We should put it after categories is defined.
}
...
fs.writeFileSync('components/PelangganDashboard.tsx', content);
