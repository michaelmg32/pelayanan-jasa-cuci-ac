...
const verifyToken = async (req, res, next) => {
  ...
  try {
    ...
    // Cross-Region Team Switching Logic
    const requestedRegionId = req.headers['x-active-region'];
    if (requestedRegionId && req.user.region_id && requestedRegionId !== String(req.user.region_id)) {
      if (req.user.role === 'admin' || req.user.role === 'keuangan') {
        let connection;
        try {
          connection = await pool.getConnection();
          const [rows] = await connection.query(`
            SELECT r2.id FROM regions r1 
            JOIN regions r2 ON r1.group_id = r2.group_id 
            WHERE r1.id = ? AND r2.id = ? AND r1.group_id IS NOT NULL
