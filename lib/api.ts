...
// ===== CUSTOMER UNIQUE AC & BARCODES =====
export const fetchCustomerACs = async (customerId?: string) => {
  try {
    ...
    console.error('Error fetching customer AC units:', error);
    return [];
  }
};

export const registerCustomerAC = async (acData: {
  id: string;
  customerId: string;
  acModelId?: string;
  brand?: string;
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(errorData.error || 'Gagal mendaftarkan AC baru.');
    }
    return await response.json();
  } catch (error) {
    console.error('Error registering customer AC:', error);
    throw error;
  }
};
...
export const fetchCustomerACHistory = async (barcodeId: string) => {
  try {
    ...
    console.error('Error fetching AC service history:', error);
    return [];
  }
};
...
export const scanCustomerAC = async (barcodeId: string) => {
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(errorData.error || 'Barcode tidak ditemukan atau tidak dapat diakses.');
    }
    return await response.json();
  } catch (error) {
    console.error('Error scanning AC barcode:', error);
    throw error;
  }
};
